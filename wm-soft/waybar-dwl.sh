...
pango_tag_active="<span                       foreground='#a6cc70'>" # Pango span style for 'active' tags
pango_tag_selected="<span                     foreground='#ff00ff'>" # Pango span style for 'selected' tags
pango_tag_urgent="<span                       background='#ed8274'>" # Pango span style for 'urgent' tags
...
_cycle() {
	...
	case "${component}" in
		...
		# Wrap component in the applicable nestable pango spans
                ...
                elif (( "${activetags}"   & mask )) 2>/dev/null; then tag_text="${pango_tag_active}${tag_text}</span>";
		else
